...
%indicated = [indic1X(ss,:)' indic1Y(ss,:)'];
%reaches = [reach1X(ss,:)' reach1Y(ss,:)'];
...
set(gcf,'color','w');
subplot(1,2,1)
histogram(sigMmarg,[0:2:60])
xline(sigma_m,'k--','LineWidth',2);
xlim([0 60])
ylim([0 100])
xlabel('Motor error \sigma_m')
ylabel('Frequency')
xticks([0:5:60])
legend('Estimates','True \sigma_m','location','northeast')
box off
set(gca, 'TickDir', 'out', 'FontSize', 18)

subplot(1,2,2)
histogram(sigPmarg,[0:2:60])
xline(sigma_p,'k--','LineWidth',2);
xlim([0 60])
ylim([0 100])
xticks([0:5:60])
xlabel('Proprioceptive error \sigma_p')
ylabel('Frequency')
legend('Estimates','True \sigma_p','location','northeast')
box off
set(gca, 'TickDir', 'out', 'FontSize', 18)

% figure 
% plot(sigma_m1,sigMmarg,'*')
% 
% figure
% plot(sigma_p1,sigPmarg,'*')

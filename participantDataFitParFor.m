...
sP = 100;
sS = 20;
sigPmax = 1:sM;                           %vector of test sigma_p values
sigMmax = 1:sP;                           %vector of test sigma_m values
sigSmax = 1:sS;                            %vector of test sigma_s values
...
load(sprintf('%s_output.mat',subj)); %loads end points, circle sizes, and marginals for sigma m&p
endPoints(136,2) = 133;
...
ub = [sigMmarg+20,100,20];                          %upper bound using marginal from control
init = [20,40,10]; %, rand*(ub(3)-lb(3))+lb(3)];                                     %initiation
options = optimoptions(@fmincon,'MaxIterations',1e5,'Display','off'); %fmincon options

m1nLogL = @(p) likelihoodFit1(p(1),p(2),p(3),numTrials,endPoints,confCirc,t,distTestEndpts,fit1LookUpMat,maxDistAll,sigMmax,sigPmax,distFromTarget,contTar,contReach,contInd);
m2nLogL = @(p) likelihoodFit2(p(1),p(2),p(3),numTrials,endPoints,confCirc,t,distTestEndpts,fit2LookUpMat,maxDistAll,sigPmax,distFromTarget,contTar,contReach,contInd);
m3nLogL = @(p) likelihoodFit3(p(1),p(2),p(3),confCirc,t,distTestEndpts,fit3LookUpMat,maxDistAll,sigMmax,distFromTarget,contTar,contReach,contInd);
...
%Calculate the BIC
BIC(1) = 2*minNLL1 + 3*log(numTrials);
BIC(2) = 2*minNLL2 + 3*log(numTrials);
BIC(3) = 2*minNLL3 + 3*log(numTrials);

BICmin = BIC - min(BIC);

%Save output
filename = sprintf('%s_dataFittingOutput.mat',subj);
save(fullfile(path,filename), 'estP1', 'estP2', 'estP3', 'minNLL1', 'minNLL2', 'minNLL3','BICmin')
